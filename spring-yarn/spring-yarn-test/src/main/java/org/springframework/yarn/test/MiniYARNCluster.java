...
import org.springframework.yarn.test.Shell.ShellCommandExecutor;
...
public class MiniYARNCluster extends CompositeService {
	...
	public void init(Configuration conf) {
		super.init(conf instanceof YarnConfiguration ? conf : new YarnConfiguration(conf));
	}
	...
	private class ResourceManagerWrapper extends AbstractService {
		...
		public synchronized void start() {
			try {
				...
				if (!getConfig().getBoolean(YarnConfiguration.YARN_MINICLUSTER_FIXED_PORTS,
						YarnConfiguration.DEFAULT_YARN_MINICLUSTER_FIXED_PORTS)) {
					// pick free random ports.
					getConfig().set(YarnConfiguration.RM_ADDRESS, MiniYARNCluster.getHostname() + ":0");
					getConfig().set(YarnConfiguration.RM_ADMIN_ADDRESS, MiniYARNCluster.getHostname() + ":0");
					getConfig().set(YarnConfiguration.RM_SCHEDULER_ADDRESS, MiniYARNCluster.getHostname() + ":0");
					getConfig()
							.set(YarnConfiguration.RM_RESOURCE_TRACKER_ADDRESS, MiniYARNCluster.getHostname() + ":0");
					getConfig().set(YarnConfiguration.RM_WEBAPP_ADDRESS, MiniYARNCluster.getHostname() + ":0");
				}
	...
	private class NodeManagerWrapper extends AbstractService {
		...
		private String prepareDirs(String dirType, int numDirs) {
			...
			for (int i = 0; i < numDirs; i++) {
				dirs[i] = new File(testWorkDir, MiniYARNCluster.this.getName() + "-" + dirType + "Dir-nm-" + index
						+ "_" + i);
				dirs[i].mkdirs();
				LOG.info("Created " + dirType + "Dir in " + dirs[i].getAbsolutePath());
				String delimiter = (i > 0) ? "," : "";
				if (Shell.WINDOWS) {
					dirsString = dirsString.concat(delimiter + "file:///" + dirs[i].getAbsolutePath().replace('\\', '/'));
				} else {
